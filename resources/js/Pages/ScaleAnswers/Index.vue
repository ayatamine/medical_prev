<template>
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                ...
                <div class="flex gap-x-2">
                    <inertia-button v-if="can.create" :href="route('admin.scale-answers.create')" classes="bg-green-100 hover:bg-green-200 text-primary"><i class="fas fa-plus"></i> New
                        Scale Answer</inertia-button>
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Scale Answer #{{currentModel.id}}</template>
                        <show-scale-answers-form :model="currentModel"></show-scale-answers-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Scale Answers
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import ShowScaleAnswersForm from "@/Pages/ScaleAnswers/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.scale-answers.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.scale-answers.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.scale-answers.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.scale-answers.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.scale-answers.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.scale-answers.update`,model.id),{
                    enabled: enabled
                }).then(res => {
