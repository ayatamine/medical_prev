<template>
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                ...
                <div class="flex gap-x-2">
                    <inertia-button v-if="can.create" :href="route('admin.scales.create')" classes="bg-green-100 hover:bg-green-200 text-primary"><i class="fas fa-plus"></i> New
                        Scale</inertia-button>
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Scale #{{currentModel.id}}</template>
                        <show-scales-form :model="currentModel"></show-scales-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Scales
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import ShowScalesForm from "@/Pages/Scales/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.scales.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.scales.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.scales.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.scales.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.scales.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.scales.update`,model.id),{
                    enabled: enabled
                }).then(res => {
